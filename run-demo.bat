...
echo ========================================
echo CourseStore Demo Launcher
echo ========================================
echo.
...
set CLIENT=build\client\Release\client.exe
...
if not exist "%CLIENT%" (
    echo ERROR: Client not found. Please run build.bat first.
    exit /b 1
)
...
echo.
echo.
echo You can now use the clients:
echo   Console Client: %CLIENT% --server 127.0.0.1 --port 8080 list
