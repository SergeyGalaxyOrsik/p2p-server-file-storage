...
echo ========================================
echo CourseStore GUI Client Launcher
echo ========================================
echo.
...
set GUI_CLIENT=build\client-gui\Release\client-gui.exe
...
if not exist "%GUI_CLIENT%" (
    echo ERROR: GUI client not found. Please run build.bat first.
    exit /b 1
)
...
echo.
echo Close this window to stop all components (Ctrl+C in each window)
echo.
